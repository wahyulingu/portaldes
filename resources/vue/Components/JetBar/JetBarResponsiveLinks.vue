<template>
    <div class="relative block lg:hidden" v-if="$page.props.jetstream.hasTeamFeatures">
        <button @click="showingSidebarManageTeamsDropdown = !showingSidebarManageTeamsDropdown"
            ...
            <svg fill="currentColor" viewBox="0 0 20 20"
                :class="{ 'rotate-180': showingSidebarManageTeamsDropdown, 'rotate-0': !showingSidebarManageTeamsDropdown }"
        ...
        <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
            ...
            <div v-if="showingSidebarManageTeamsDropdown"
                ...
                <div class="px-2 py-2 bg-white rounded-md shadow dark-mode:bg-gray-800">
                    ...
                    <Link :href="route('teams.show', $page.props.auth.user.current_team)"
                        :class="route().current('teams.show') ? 'bg-gray-200' : 'bg-transparent'"
                        class="block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:hover:bg-gray-600 dark-mode:focus:bg-gray-600 dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:text-gray-200 lg:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline">
                    Team
                    Settings</Link>
                    <Link :href="route('teams.create')"
                        :class="route().current('teams.create') ? 'bg-gray-200' : 'bg-transparent'"
    ...
    <div v-show="showingSidebarManageAccountDropdown" @click="showingSidebarManageAccountDropdown = false"
        class="fixed inset-0 h-full w-full z-10" style="display: none;"></div>

    <div class="relative block lg:hidden">
        <button @click="showingSidebarManageAccountDropdown = !showingSidebarManageAccountDropdown"
            class="flex flex-row items-center w-full px-4 py-2 mt-2 text-sm font-semibold text-left bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:focus:bg-gray-600 dark-mode:hover:bg-gray-600 lg:block hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline">
            <span>Manage Account</span>
            <svg fill="currentColor" viewBox="0 0 20 20"
                :class="{ 'rotate-180': showingSidebarManageAccountDropdown, 'rotate-0': !showingSidebarManageAccountDropdown }"
                class="inline w-4 h-4 mt-1 ml-1 transition-transform duration-200 transform lg:-mt-1">
                <path fill-rule="evenodd"
                    d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"
                    clip-rule="evenodd"></path>
        ...
        <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
            enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75"
            leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
            <div v-if="showingSidebarManageAccountDropdown"
                class="absolute right-0 w-full mt-2 origin-top-right rounded-md shadow-lg z-20">
                <div class="px-2 py-2 bg-white rounded-md shadow dark-mode:bg-gray-800">
    ...
    <div v-show="showingSidebarManageTeamsDropdown" @click="showingSidebarManageTeamsDropdown = false"
        class="fixed inset-0 h-full w-full z-10" style="display: none;"></div>
</template>
...
const showingSidebarManageAccountDropdown = ref(false)
const showingSidebarManageTeamsDropdown = ref(false)
